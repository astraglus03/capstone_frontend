...
class _HomeScreenState extends ConsumerState<HomeScreen>
  ...
  void _updateMonth(int change) {
    ...
    setState(() {
      ...
      daysInMonth = _daysInMonth(currentDate);
      ref.read(monthListProvider.notifier).fetchDiariesForMonth(userId.toString(), DateFormat('yyyy-MM').format(currentDate));
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(monthListProvider);
    final state1 = ref.watch(representMonthEmotionProvider);
    ...
    return Scaffold(
        ...
        title: Row(
          children: [
            state1.when(
              ...
              error: (error, _) => Text('대표 감정 에러: $error'),
              loading: () => _buildMonthEmotionSkeletonUI(),
            ),
      ...
      body: Column(
        children: [
          ...
          state.when(
            data: (diaries) => CarouselSlider.builder(
              itemCount: daysInMonth,
              carouselController: carouselController,
              options: CarouselOptions(
                height: 440,
              ...
              itemBuilder: (context, index, realIndex) {
                ...
                List<DiaryModel> filteredDiaries = diaries
                    .where((diary) =>
                ...
                // 첫 번째 일기 선택
                DiaryModel? dayDiary =
                filteredDiaries.isNotEmpty ? filteredDiaries.first : null;
            ...
            loading: () => _buildContainerSkeletonUI(),
          ),
        ],
      ),
    );
